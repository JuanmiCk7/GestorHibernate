...
public class ModifyActivityController implements Initializable {
	...
	/**
	 * Boton con el que incluimos una imagen (no funcional)
	 */
	@FXML
	private Button btnAddImage;

	/**
	 * Boton para incluir la actividad a la base de datos
	 */
	@FXML
	...
	/**
	 * Metodo utilizado para volver a la anterior pantalla.
	 * 
	 * @param event
	 */
	@FXML
	...
	/**
	 * Metodo utilizado para a�adir la actividad en la base de datos.
	 * @param event
	 */
	@FXML
	private void onClickAddActivity(MouseEvent event) {
		...
		try {
			...
			if (!activityName.isEmpty() && !activityDescription.isEmpty()) {	
				ActividadesDao.updateActivity(ActividadesDao.getSelectedActivity(), activityName, activityDescription, sqlDate);
				Node node = (Node) event.getSource();
				Stage stage = (Stage) node.getScene().getWindow();
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		textNombreActividad.setText(ActividadesDao.searchActivitiesWithId(ActividadesDao.getSelectedActivity()).getName());
		textDescripcionActividad.setText(ActividadesDao.searchActivitiesWithId(ActividadesDao.getSelectedActivity()).getDescription());
		datePicker.setPromptText(ActividadesDao.searchActivitiesWithId(ActividadesDao.getSelectedActivity()).getDate().toString());
	}
